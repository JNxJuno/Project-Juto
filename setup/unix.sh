...
# 1. Die Bibliotheken herunterladen (P-Slice Git Versionen)
...
# 2. DER FIX (Das Skalpell):
# Wir suchen in der Application.hx nach "super(appMeta);" und 
# ändern es in "super();", weil Lime das Argument nicht mehr will.
sed -i 's/super(appMeta);/super();/g' .haxelib/openfl/git/src/openfl/display/Application.hx

# 3. Den Rest installieren
