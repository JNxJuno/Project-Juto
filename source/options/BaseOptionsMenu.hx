...
import flixel.FlxCamera; // WICHTIG: Für den Slide-Effekt
...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	public var slideCam:FlxCamera; // NEU: Unsere eigene Kamera zum Sliden
	...
	public function new()
	{
		...
		// ✨ NEU: Kamera erstellen, links aus dem Bildschirm setzen und reinsliden lassen
		slideCam = new FlxCamera();
		slideCam.bgColor.alpha = 0;
		slideCam.x = -FlxG.width; 
		FlxG.cameras.add(slideCam, false);
		FlxTween.tween(slideCam, {x: 0}, 0.5, {ease: FlxEase.expoOut});
		...
		bg.alpha = 0; // Unsichtbar für sauberes UI
		add(bg);

		grpOptions = new FlxTypedGroup<Alphabet>();
		grpOptions.cameras = [slideCam]; // Auf die Slide-Kamera packen
		add(grpOptions);

		grpTexts = new FlxTypedGroup<AttachedText>();
		grpTexts.cameras = [slideCam]; // Auf die Slide-Kamera packen
		add(grpTexts);

		checkboxGroup = new FlxTypedGroup<CheckboxThingie>();
		checkboxGroup.cameras = [slideCam]; // Auf die Slide-Kamera packen
		add(checkboxGroup);

		descBox = new FlxSprite().makeGraphic(1, 1, FlxColor.BLACK);
		descBox.alpha = 0;
		descBox.cameras = [slideCam];
		add(descBox);

		var titleText:Alphabet = new Alphabet(75, 45, title, true);
		titleText.setScale(0.6);
		titleText.alpha = 0;
		titleText.cameras = [slideCam];
		add(titleText);

		var txtWidthOffset:Float = Math.max(MobileScaleMode.gameCutoutSize.x / 2,50);

		descText = new FlxText(txtWidthOffset, FlxG.height - 100, FlxG.width-(txtWidthOffset*2), "", 32);
		descText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		descText.scrollFactor.set();
		descText.borderSize = 2.4;
		descText.cameras = [slideCam]; // Beschreibung slidet auch mit rein
		add(descText);
	...
	// ✨ NEU: Kamera ordentlich löschen, wenn wir zurückgehen
	override function destroy()
	{
		if (slideCam != null) {
			FlxG.cameras.remove(slideCam);
		}
		super.destroy();
	}
	...
	function updateBind(?text:String = null, ?option:Option = null)
	{
		...
		attach.x = bind.x;
		attach.y = bind.y;
		attach.cameras = [slideCam]; // Neu erstellter Text bekommt auch die Slide Kamera
