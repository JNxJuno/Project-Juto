...
class MusicBeatState extends FlxState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.P && !Std.isOfType(this, mikolka.vslice.ui.MusicSettingsSubState)) {
			if (subState == null) {
				openSubState(new mikolka.vslice.ui.GlobalPlaylistSubState());
			} else if (Std.isOfType(subState, mikolka.vslice.ui.GlobalPlaylistSubState)) {
				subState.close();
			}
		}

		// ✨ Speichert Zeit UND den Namen des Songs ab!
		var stateName:String = Type.getClassName(Type.getClass(this));
		if (stateName.indexOf("PlayState") == -1 && FlxG.sound.music != null && FlxG.sound.music.playing) {
			saveTimeTimer += elapsed;
			if (saveTimeTimer >= 2.0) { 
				FlxG.save.data.lastSongTime = FlxG.sound.music.time;
				FlxG.save.data.lastSongName = mikolka.vslice.ui.MusicSettingsSubState.currentPlayingFile;
				FlxG.save.flush();
				saveTimeTimer = 0;
			}
		}
