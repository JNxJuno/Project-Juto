...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// Erzwingt den smoothen Übergang beim Betreten
		...
		for (i => credit in creditsStuff)
		{
			...
			optionText.color = flixel.util.FlxColor.WHITE;
	...
	override function update(elapsed:Float)
	{
			...
			if (controls.BACK)
		{
			...
			quitting = true; // Verhindert weiteren Input

			// ✨ CREDITS ZOOM-OUT & FADE ✨
			flixel.tweens.FlxTween.tween(FlxG.camera, {zoom: 0.7}, 0.5, {ease: flixel.tweens.FlxEase.expoIn});
